...
- [ ] **`../comapeo-docs` uncommitted workflow drift** (flag for that repo, not this one): local edits to both deploy workflows revert `static/images/` → `static/img/`, which would break deploy of pipeline-published images under `static/images/notion/`. Confirm intent and commit or discard.
